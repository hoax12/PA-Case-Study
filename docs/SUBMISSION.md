...
because a model's answer cannot be audited, can change between runs, and — the
worst case — could wrongly deny care. News coverage of AI-driven bulk denials
...
There is no third outcome. A denial is not "discouraged by the prompt" — it is
**unrepresentable**: the outcome type in the code has two values, so no model
error, bug, or prompt injection can produce a denial. A denial can enter the
record in exactly one way: a person posts it through the review screen, and it
is stored with their name on it.
...
- **Every quote must be real.** If the model cites a sentence that cannot be
  found in the transcribed documents, that evidence is discarded and the
  criterion becomes "unknown." A hallucinated quote — or text an attacker
  hides inside an uploaded document — cannot become a fact.
...
**I built the first version of this project the other way** — the standard RAG
recipe: chop the PDF into ~300-word chunks, embed them, search by similarity.
...
all upfront. Each catalog-informed miss is logged with the service name — the
production traffic itself writes the authoring backlog. Adding a policy is
zero code: a script sends the payer PDF to the model, which drafts the YAML;
a verifier then flags every drafted clause it cannot find verbatim on its
cited page; a human reviews **only the flags** and renames the file to
activate it. Until that rename, the registry refuses to load it — a
machine-drafted rule can never decide coverage unreviewed.
...
outcome type cannot express a denial" — are only credible when executable.
...
| Evidence quotes verified against transcripts | 78/78 |
...
approved) — the safety asymmetry working as designed, but also proof that
...
In order: author the second criteria policy from the catalog backlog to
measure the marginal-policy cost for real; run an adversarial evaluation
(prompt-injection attempts inside uploaded documents against the quote gate);
replace hand-set confidence with reviewer-agreement calibration; and add the
targeted evidence-search loop for unknown criteria — the one place an agent
